...
import (
	...
	"github.com/AshimBasumatary/task"
)
...
func GetPolicyReq(i, r, g, AP int) string {
	now := time.Now().Unix()
	policyStr := fmt.Sprintf(`{"AS":{"userId":"%s","role":"u%d","group":"g%d"},"AO":{"deviceId":"%s","MAC":"%s"},"AP":%d,"AE":{"startTime":%d,"endTime":%d"}}`, GetUserID(i), r, g, GetDeviceID(i), RandomMac(), AP, now, now+100000)
	return policyStr
}
